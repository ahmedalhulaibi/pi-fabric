...
const sourceLabel = (sourceInfo: SourceInfo): string => {
	...
	const packageSegment = [...segments]
		.reverse()
		.find((segment) => segment.startsWith("pi-"));
...
export class CapturedToolsProvider implements FabricProvider {
	...
	prepareArguments(
		...
		const prepare =
			this.catalog.require(actionName).wrappedTool.prepareArguments;
		...
		if (
			...
			throw new Error(
				`Captured tool ${actionName} prepared non-object arguments`,
			);
		}
	...
	async invokeCapturedTool(
		...
		return this.invoke(actionName, args, {
			...context,
	...
	async #invokeCaptured(
		...
		try {
			...
			if (preflight?.block) {
				throw new Error(
					preflight.reason || `Captured tool ${entry.name} was blocked`,
				);
			}
			result = await runAbortable(context.signal, () =>
				wrappedTool.execute(
					...
					(partialResult) => {
						...
						if (progress)
							context.update(`${entry.name}: ${progress.slice(0, 500)}`);
			...
			throw new Error(
				text ||
					(thrown instanceof Error
						? thrown.message
						: `Captured tool ${entry.name} failed`),
			);
		}
