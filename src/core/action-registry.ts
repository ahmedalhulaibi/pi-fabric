...
export interface FabricRegistryInvocationContext
	extends FabricInvocationContext {
...
const previewResult = (value: unknown): unknown => {
	if (typeof value === "string")
		return truncateString(value, PREVIEW_RESULT_CHARS);
...
const failedResultError = (value: unknown): string | undefined => {
	...
	return error
		? truncateString(error, PREVIEW_RESULT_CHARS)
		: `Fabric action returned ${status}`;
};

const failedResultOutcome = (
	value: unknown,
...
const boundedResult = (
	...
	try {
		...
		throw new Error(
			`Fabric action returned a non-JSON-serializable value: ${message}`,
		);
	}
...
const discoveryTerms = (value: string): string[] =>
	[...value.normalize("NFKC").matchAll(/[\p{L}\p{N}_]+/gu)].map((match) =>
		match[0].toLowerCase(),
	);
...
export class ActionRegistry {
	...
	constructor(
		readonly toolResultProxy?: FabricNestedToolResultProxy,
		private readonly invokeCapturedTool?: (
			name: string,
			args: Record<string, unknown>,
		...
		const lists = await Promise.all(
			providers.map(async (provider) => {
				...
				return descriptors.map((descriptor) =>
					resolveDescriptor(provider, descriptor),
				);
			}),
		);
	...
	async catalog(
		...
		const lists = await Promise.all(
			providers.map(async (provider) => ({
				provider,
				actions: (await provider.list({}, context)).map((descriptor) =>
					resolveDescriptor(provider, descriptor),
				),
			})),
		);
		...
		const limit = Math.max(
			1,
			Math.min(Math.floor(options.limit ?? 1_000), 1_000),
		);
		const retainedRefs = new Set(
			allActions.slice(0, limit).map((action) => action.ref),
		);
		...
		const rootHash = descriptorHash(
			providerHeads.map((provider) => provider.descriptorHash),
		);
		return {
		...
		return listed
			.map((action) => {
				const providerDescription =
					this.#providers.get(action.provider)?.description ?? "";
				...
				const providerBody = providerDescription
					.normalize("NFKC")
					.toLowerCase();
			...
			.sort(
				(left, right) =>
					...
					left.action.ref.localeCompare(right.action.ref),
			)
	...
	async describe(
		...
		for (const provider of this.#providers.values()) {
			...
			for (const descriptor of descriptors) {
				if (descriptor.name === ref)
					matches.push(resolveDescriptor(provider, descriptor));
			}
		}
	...
	async invoke(
		...
		const traceOperation =
			context.traceOperation ?? context.trace?.issueCall(ref, args);
		let failureStage:
			| "resolve"
			| "guard"
			| "prepare"
		...
		try {
			...
			if (
				...
				throw new Error(
					`Argument preparation for ${ref} did not return an object`,
				);
			}
			...
			await runAbortable(context.signal, () =>
				context.approve(action, preparedArgs),
			);
			...
			const activeAudit: FabricCallAudit = {
				...
				args: boundedPreviewValue(argsPreview, MAX_AUDIT_VALUE_CHARS) as Record<
					string,
					unknown
				>,
			};
			...
			const providerValue = await runAbortable(context.signal, () =>
				provider.invoke(actionName, preparedArgs, {
					...context,
					nestedToolCallId,
					...(this.invokeCapturedTool
						? {
								invokeCapturedTool: (
									name: string,
									args: Record<string, unknown>,
								) => this.invokeCapturedTool!(name, args, context),
							}
						: {}),
					update(message) {
			...
			activeAudit.result = boundedPreviewValue(
				resultPreview,
				MAX_AUDIT_VALUE_CHARS,
			);
			...
			traceOperation?.fail(
				failureStage,
				error,
				executionOutcomeFromError(error, context.signal),
			);
			if (audit) {
				audit.success = false;
				audit.error = message;
	...
	#parseRef(ref: string): { provider: FabricProvider; actionName: string } {
		...
		if (separator <= 0 || separator === ref.length - 1) {
			throw new Error(
				`Fabric action references must use provider.action: ${ref}`,
			);
		}
